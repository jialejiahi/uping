...
ping 是测试连通性和时延的工具,简单易用, 但有时候我们需要使用 udp/tcp协议,不能用 icmp。

开源工具 nc, iperf,sockperf 等,可以测连通性和 vm2vm 场景的时延.

但上面的工具都没考虑负载均衡多个rs的场景,用起来也没ping那么简单直观。

因此开发一个测试工具 uping, 基本功能是测试 udp/tcp 连通性,并能够像系统自带的 ping 一样统计 rtt 时延。
...
6. 需要能够并发测试多个server端口，打多条流

7. 需要能够指定长连接或短连接模式

8. 需要做一些基本的系统优化,比如调大 socket buffer 大小适配突发
...
| -T   | 使用TCP协议，使用TCP时客户端尽量同时设置-m参数         | False   |
| -N   | 开启NoDelay,默认开启，开启后会关闭内核的Nagle算法，不合并小包发送  | True   |
| -Q   | 开启Tcp_QuickAck选项, Ack 包将单独发送, 不等待发送数据时携带   | False   |
...
| -p   | bind 本地端口,不设置时随机选择                   | 默认为 0,随机选择     |
| -m   | mutable connection 短连接方式发包,每次发包后断开连接 | 默认为 0,长连接    |
| -l   | length, 负载的长度,最小取值 64 字节以容纳包头     | 64                     |
| -L   | upper payload length limit, 负载长度递增时的最大长度 | 0 |
| -i   | interval, 发包间隔, 单位 ms                     | 100                    |
| -c   | count,发包数量                                 | 10                     |
| -t   | timeout, 一个报文无应答的超时时间,单位 ms        |  1000                   |
| -E   | 使用SSL协议加密数据，仅支持客户端, 服务端可以通过代理实现SSL通信       | False   |
| -S   | strict check flag, 客户端发固定内容, 服务端严格检查payload所有字节  | False   |
...
2. 使用request id的最后一个bit表示客户端是否使用mutable connection
